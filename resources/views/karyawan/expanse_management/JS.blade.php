<script>
    $(function () {
        ...
        let table = $('.data-table').DataTable({
            ...
            serverSide: true,
            responsive: true,
            ajax: "{{ route('karyawan.expanses.index') }}",
            columns: [
                ...
                {data: 'action', name: 'action', orderable: false, searchable: false,
                    render: function( data, _type, _full ) {
                        let btn = null;
                        btn = '<button type="button" data-id="/api/karyawan/expanses/' + data + '" class="viewBtn btn btn-gradient-success btn-sm mr-1" title="view"><i class="mdi mdi-eye menu-icon"></i></button>';
                        return btn;
                }},
            ]
        });
