<script>
    $(function(){
        ...
        let tablePackage = $('#table-package').DataTable({
            ...
            serverSide: true,
            responsive: true,
            ajax: "{{ route('karyawan.packages.index') }}",
            columns: [
        ...
        let tableDetailTr = $('#table-detailTr').DataTable({
            ...
            ajax: {
                'type': 'GET',
                'url': '{{ route("karyawan.transaction-details.index") }}',
            ...
            columns: [
                ...
                {data: 'action', name: 'action', orderable: false, searchable: false,
                    render: function( data, _type, _full ) {
                        ...
                        btn = '<button data-id="/api/karyawan/transaction-details/' + data.id + '" class="removeItemBtn btn btn-gradient-danger btn-sm mb-1"><i class="mdi mdi-delete menu-icon"></i></button>';
        ...
        let tablePrintMark = $('#table-print-mark').DataTable({
            ...
            ajax: {
                'type': 'GET',
                'url': '{{ route("karyawan.transaction-details.indexMark") }}',
                'data': { id_transaction: '{{ $transaction->id }}' }
            },
            columns: [
                {data: 'DT_RowIndex', name: 'DT_RowIndex'},
        ...
        let Vdef = {
            ...
            'url'           : "/api/karyawan/transaction-details/",
            'urlStatusItem' : '/api/karyawan/transaction-details/update-status/'
        };
